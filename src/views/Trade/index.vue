<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      >
        <span
          :class="{ username: true, selected: userId === address.id }"
          @click="userId = address.id"
    ...
    <div class="trade">
      <div class="price">
        应付金额:<span>¥{{ trade.totalAmount }}0</span>
      </div>
      <div class="receiveInfo">
        寄送至:
        <span>{{ userTradeList.userAddress }}</span>
        收货人：<span>{{ userTradeList.consignee }}</span>
        <span>{{ userTradeList.phoneNum }}</span>
      </div>
    </div>
...
<script>
import { reqGetTrade, reqSubmitOrder } from "@api/trade";
...
export default {
  ...
  computed: {
    userTradeList() {
      ...
      const a = userAddressList
        ? userAddressList.find((address) => address.id === userId)
        : {};
      console.log(a);
      return a;
    },
  ...
  async mounted() {
    ...
    this.trade = trade;
    this.userId = trade.userAddressList.find(
